...
namespace WpfPosApp
{
    public partial class frmSaleReceipt : Form
    {
        ...
        private void FrmSaleReceipt_Load(object sender, EventArgs e)
        {
            ...
            {
                cn.Open();
                SqlCommand cmd = new SqlCommand("select c.TransDetID, c.ProdID, c.price, c.qty, c.total_price, c.added_date, p.Full_Name from TransDetails as c inner join Product as p on p.ProdID = c.ProdID where transno like '" + sale.lblTransNoUnit.Content + "'", cn);
